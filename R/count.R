...
# The counts column of the output will be called 'count' unless this clashes with another column name, in which case it will be renamed to something like 'count..1'
...
# .drop is either FALSE (default) to include output rows with a count of zero, or TRUE to exclude them; if FALSE, there will be a row for each unique combination of values and NA factor levels among the counted columns (including NA values and factor levels, which will not be distinguished from one another)
}
