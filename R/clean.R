...
# The objects given by '...' will be any other arguments to whatever function is being used to read the file (NB: these don't always have the same names in different functions, so if you do specify read_function then all the arguments will be passed in with partial matching supported, but if you do not specify read_function then only those options that have the right names will be passed in and partial matching will not be supported - if you are calling this function repeatedly on a mixture of Excel and CSV files with the same options where names are the same, you can pass in the options for both types and they will be used appropriately, e.g. you can say which values should be changed to NA by passing in 'na' and 'na.strings' and it will be understood by 'read_excel', 'fread', 'read_csv' and 'read.csv', and you can pass in both 'header' and 'col_names' to specify whether a header row should be read in 'fread', 'read.csv', 'read_excel' and 'read_csv')
